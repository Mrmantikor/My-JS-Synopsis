...
*/
/* //! Higher-Order Function (Функція вищого порядку)
TODO: Функція, яка приймає іншу функцію як аргумент або повертає функцію як результат.
 * - Функція вищого порядку (HOF) — це функція, яка працює з іншими функціями як зі звичайними значеннями.
 * - Може приймати колбек (функцію-аргумент), щоб виконати її в потрібний момент.
 * - Може повертати нову функцію, яку можна викликати пізніше.
 * - Не змінює зовнішній код напряму, але залежить від того, що роблять передані чи повернуті функції.
 * - Використовується у методах масивів (map, filter, forEach), для налаштування поведінки коду.

TODO: Приклад 1. Функція вищого порядку, яка приймає колбек
function executeAction(action) {
  console.log('Починаємо...');
  action(); // Викликаємо передану функцію (колбек)
}
function sayDone() {
  console.log('Готово!');
}
executeAction(sayDone); // Виводить: 'Починаємо...' і 'Готово!'

TODO: Приклад 2. Функція вищого порядку, яка повертає функцію
function createMultiplier(multiplier) {
  return function (number) {
    return number * multiplier;
  };
}
const triple = createMultiplier(3); // Повертає функцію множення на 3
console.log(triple(4)); // Виводить: 12

TODO: Приклад 3. Функція, яка приймає і повертає функцію
function modifyNumber(number, operation) {
  return operation(number); // Викликає передану функцію (колбек)
}
function addTen(num) {
  return num + 10;
}
console.log(modifyNumber(5, addTen)); // Виводить: 15
 ...
 * - Виконується після завершення певної дії (синхронної, без затримок).
 * - Не має власного часу виклику, залежить від функції, якій її передали.
 * - Часто використовується в методах масиву (map, filter) та обробці даних.
 * - Корисно для гнучкої роботи з різними операціями.
...
TODO: Приклад 1. Обробка замовлення в магазині
function processOrder(order, callback) {
  console.log(`Обробляємо замовлення: ${order}`);
  callback(order);
}

function confirmOrder(order) {
  console.log(`Замовлення "${order}" підтверджено!`);
}

processOrder('Смартфон', confirmOrder);
  Виводить: "Обробляємо замовлення: Смартфон"
  Виводить: "Замовлення 'Смартфон' підтверджено!"

TODO: Приклад 2. Callback з параметрами (числові операції)
function processNumber(num, callback) {
  const result = num * 2;
  callback(result); // Викликаємо колбек із результатом
}
...
TODO: Приклад 3. Послідовна обробка числа через кілька функцій
function processNumber(number, callbacks) {
  let result = number; 
...
const operations = [addFive, multiplyByThree, subtractTwo];
console.log(processNumber(2, operations));
  Виводить: 19  (Обчислення: ((2 + 5) * 3) - 2 = 19)
*/
...
console.log(processNumber(3, square)); // 9
*/
/* //! processNumber (Обробка числа через три колбек-функції)
TODO: Функція, яка приймає число та масив колбек-функцій, обробляє це число за допомогою кожного колбеку і повертає результат.
 * - Використовує масив колбек-функцій для послідовної обробки числа.
 * - Пройде через кожну функцію з масиву і застосує її до результату попереднього кроку.
 * - Дозволяє динамічно змінювати логіку обробки числа, додаючи або змінюючи колбеки.
 * - Гнучкість коду завдяки використанню колбеків для виконання різних операцій.
 * - Ідеально підходить для складних обчислень, що включають кілька операцій.

TODO: Task
function processNumber(number, callbacks) {
}

console.log(processNumber(2, operations)); // Виводить: 19 ( ((2 + 5) * 3) - 2 )
